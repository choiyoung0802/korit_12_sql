...
## DELETE : 데이터 삭저
* 참조 사항 : 마찬가지로 `WHERE` 절을 이용해 특정 row 만 삭제해야 합니다. 고유값(PK)의 의미가 중요한 이유입니다.
```sql
-- DELETE
...
1. 이하의 테이블들을 생성하시오.
    ...
    - enrollments
        - enrollment_id : 자동증가 적용 / pk 적용 / int
...
2. 더미 데이터 삽입
    ...
    - enrollments의 student_id / course_id / enrollment_date 에
        - 1, 1, '2026-02-01'
        - 1, 2, '2026-02-01'
        - 2, 1, '2026-02-02'
...
INSERT INTO courses (course_name, professor, credits)
  ...
  -- default 가 3으로 잡혀있어서 3 굳이 안 넣어도 됨
...
- 이상의 경우 sql 문만 봤을 때 오류가 발생하는지 여부를 확실할 수 없습니다.
...
* FK 제약 조건이 걸려있는 테이블을 sql 문으로 생성하기
...
- AppUser 클래스를 정의하시오.
    ...
    2. 생성자를 만드시오(JPA 에서는 NoArgsConstructor 가 필수적입니다.)
    3. SpringBoot 프로젝트를 실행시키고, 
      - id, name, username, password 순으로 1, 김일, kim1@test.com, kim123456 으로 row 를 삽입하시오.
      - SELECT * FROM APP_USER; 를 통해 row 가 들어가있음을 확인하시오.
      - 캡처해서 md 파일에 옮겨놓고 자습
![성공](image-10.png)